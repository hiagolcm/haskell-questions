...
Concepts: arithmetic operators & precedence, negative numbers, boolean
operators (`&&`, `||`, `not`), `if/then/else`, custom function definitions,
infix (backtick) function calls.
...
Given a Celsius temperature, classify it using `if/then/else` (every branch
must return, there is no such thing as an `if` without `else` in Haskell):
...
the humidity is between 30 and 60 (inclusive). Combine both conditions with
`&&`, and build each range check with `||`/comparisons as needed.
